...
export default function About() {
	const [modeStyle,setModeStyle] = useState({backgroundColor: 'white',color: 'black'});
	const [btnModeChange,setBtnModeChange] = useState("Enable Dark Mode");
	const toggleStyle = () => {
		if(modeStyle.color==='black')
		{
			setModeStyle({backgroundColor: 'black',color: 'white'});
			setBtnModeChange("Enable Light Mode");
		}
		else
		{
			setModeStyle({backgroundColor: 'white',color: 'black'});
			setBtnModeChange("Enable Dark Mode");
		}
  ...
  return (
    <div className='container' style={modeStyle}>
		...
		<div className="container py-2">
			<button onClick={toggleStyle} className='btn btn-primary'>{btnModeChange}</button>
		</div>
    </div>
  )
}
